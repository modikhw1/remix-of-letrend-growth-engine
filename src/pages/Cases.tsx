            ...
            {filtered.map((c, i) => (
              <FadeIn key={c.slug} delay={i * 0.05}>
                <Link to={`/kundcase/${c.slug}`} className="group block">
                  <div className="flex h-full flex-col rounded-2xl border-thicker border-foreground bg-card p-7 shadow-hard transition-all group-hover:shadow-hard-lg group-hover:-translate-y-1">
