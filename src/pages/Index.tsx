...
const Index = () => {
  return (
    <Layout>
        ...
        <div className="container relative z-10">
          <FadeIn className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              ...
              <Button asChild variant="outline" size="lg" className="rounded-full border-thicker border-foreground bg-background px-8 shadow-hard-sm transition-all hover:shadow-hard">
        ...
        <div className="container">
          <div className="grid gap-5 md:grid-cols-3">
            ...
            <div className="flex flex-col gap-5">
              <FadeIn delay={0.1}>
                <div className="rounded-2xl border-thicker border-foreground bg-accent/15 p-6 shadow-hard-sm">
              ...
              <FadeIn delay={0.2}>
                <div className="rounded-2xl border-thicker border-foreground bg-gold/50 p-6 shadow-hard-sm">
            ...
            <FadeIn delay={0.2} className="md:col-span-2">
              <div className="flex items-center justify-between rounded-2xl border-thicker border-foreground bg-brand p-8 text-brand-foreground shadow-hard-accent">
                <div>
                  ...
                  <p className="mt-2 text-sm opacity-80">{content.featureBento.problemBody}</p>
                  <Link to="/hur-det-fungerar" className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-accent hover:underline">
      ...
      {/* ═══ STATS ═══ */}
      <section className="border-b-2 border-foreground bg-accent/10 py-14">
        <div className="container">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {content.positioning.columns.map((col, i) => (
              <FadeIn key={col.title} delay={i * 0.1}>
                <div className={`h-full rounded-2xl border-thicker border-foreground p-7 shadow-hard ${col.highlight ? "bg-gold" : "bg-card"}`}>
                  <h3 className={`text-xl font-bold mb-5 ${col.highlight ? "text-foreground" : ""}`}>{col.title}</h3>
                  <ul className="space-y-3 text-sm">
                    {col.items.map((item) => (
                      <li key={item} className={`flex items-start gap-2 ${col.highlight ? "" : "text-muted-foreground"}`}>
                        {col.highlight && <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-foreground" />}
                        {item}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="container relative z-10">
          <div className="grid items-center gap-12 md:grid-cols-2">
            <FadeIn>
              ...
              <p className="mt-4 opacity-80">{content.aiHuman.body}</p>
              <div className="mt-8 grid grid-cols-2 gap-3">
                {content.aiHuman.features.map((item, idx) => {
                  const Icon = aiIcons[idx];
                  return (
                    <div key={item.label} className="rounded-xl border-2 border-white/30 bg-white/10 p-4 backdrop-blur-sm">
            ...
            <FadeIn delay={0.1} className="order-1 md:order-2">
              ...
              <div className="mt-8 space-y-3">
                {content.platformPreview.modules.map((m, idx) => {
                  ...
                  return (
                    <div key={m.title} className="flex items-center gap-3 rounded-xl border-thicker border-foreground/20 bg-card px-4 py-3 transition-all hover:border-foreground hover:shadow-hard-sm">
      ...
      {/* ═══ PROCESS ═══ */}
      <section className="border-b-2 border-foreground bg-blush/60 py-20 md:py-28">
        <div className="container">
          ...
          <FadeIn delay={0.1}>
            <div className="mx-auto max-w-4xl grid gap-4 md:grid-cols-5">
              {content.process.steps.map((s, idx) => (
                <div key={s.num} className={`rounded-2xl border-thicker border-foreground ${stepColors[idx]} p-5 text-center shadow-hard-sm transition-all hover:shadow-hard hover:-translate-y-0.5`}>
          ...
          <div className="mt-10 text-center">
            <Button asChild variant="outline" className="rounded-full border-thicker border-foreground bg-background px-8 shadow-hard-sm hover:shadow-hard transition-all">
              ...
              <FadeIn key={c.slug} delay={i * 0.1}>
                <Link to={`/kundcase/${c.slug}`} className="group block">
                  <div className="h-full rounded-2xl border-thicker border-foreground bg-card p-7 shadow-hard transition-all group-hover:shadow-hard-lg group-hover:-translate-y-1">
                    ...
                    <p className="mt-3 inline-block rounded-lg bg-accent/10 border border-accent/20 px-3 py-1.5 text-sm font-semibold text-accent">{c.stat}</p>
      ...
      <section className="border-b-2 border-foreground bg-card py-20 md:py-28">
        <div className="container">
          <FadeIn className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-muted-foreground">{content.pricingTeaser.body}</p>
          </FadeIn>
          <FadeIn delay={0.1}>
          ...
          <div className="mx-auto mt-14 max-w-2xl">
            <Accordion type="single" collapsible className="space-y-3">
              {content.faq.items.map((item, i) => (
                <AccordionItem key={i} value={String(i)} className="rounded-xl border-thicker border-foreground px-6 bg-card shadow-hard-sm">
      ...
      <section className="relative border-b-2 border-foreground bg-accent py-24 text-accent-foreground md:py-32 overflow-hidden">
        <FloatingDecorations layout="cta" light />
        <div className="container relative z-10">
          <FadeIn className="mx-auto max-w-2xl text-center">
            ...
            <Button asChild size="lg" className="mt-8 rounded-full border-2 border-foreground bg-foreground text-background shadow-hard px-10 transition-all active:shadow-none active:translate-x-[4px] active:translate-y-[4px]">
              <Link to="/kom-igang">{global.cta.primary}</Link>
            </Button>
