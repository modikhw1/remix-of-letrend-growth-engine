...
interface GumballDecorationsProps {
  layout: string;
  /** Use on dark backgrounds — renders with warm bg circle behind each item */
  light?: boolean;
}

const L = logoMark; // shorthand for logo

const layouts: Record<string, DecoItem[]> = {
  "hero-home": [
    { src: foodCooking, className: "absolute -left-8 top-16 h-28 w-28 md:h-40 md:w-40", rotate: -12 },
    { src: L, className: "absolute right-[5%] bottom-8 h-16 w-16 md:h-24 md:w-24", rotate: 10, delay: "1.5s" },
  ...
  "hero-platform": [
    ...
    { src: L, className: "absolute right-[8%] bottom-12 h-14 w-14 md:h-20 md:w-20", rotate: 12, delay: "2s" },
  ...
  "hero-cases": [
    ...
    { src: L, className: "absolute left-[5%] bottom-8 h-16 w-16 md:h-24 md:w-24", rotate: -8, delay: "1s" },
  ],
  "hero-about": [
    { src: waving, className: "absolute -left-8 top-20 h-28 w-28 md:h-36 md:w-36", rotate: -10 },
    { src: L, className: "absolute right-[6%] bottom-12 h-16 w-16 md:h-24 md:w-24", rotate: 12, delay: "2s" },
  ],
  "hero-howit": [
    { src: startMono, className: "absolute right-[8%] top-16 h-20 w-20 md:h-28 md:w-28", rotate: 8 },
    { src: L, className: "absolute -left-6 bottom-12 h-16 w-16 md:h-20 md:w-20", rotate: -12, delay: "1.5s" },
  ],
  "ai-section": [
    { src: dataChart, className: "absolute -right-6 top-8 h-24 w-24 md:h-32 md:w-32", rotate: 10 },
    { src: L, className: "absolute left-[3%] bottom-6 h-14 w-14 md:h-20 md:w-20", rotate: -15, delay: "2s" },
  ...
  "tiktok-section": [
    ...
    { src: L, className: "absolute right-[4%] top-[20%] h-14 w-14 md:h-20 md:w-20", rotate: 15, delay: "2.5s" },
  ...
  "cta-section": [
    ...
    { src: L, className: "absolute -right-4 bottom-4 h-16 w-16 md:h-24 md:w-24", rotate: 20, delay: "2s" },
  ],
  "case-detail": [
    { src: audience, className: "absolute -right-6 top-12 h-24 w-24 md:h-32 md:w-32", rotate: 8 },
    { src: L, className: "absolute left-[4%] bottom-8 h-12 w-12 md:h-16 md:w-16", rotate: -10, delay: "1.5s" },
  ],
  "process-section": [
    { src: musicSound, className: "absolute -right-4 top-8 h-20 w-20 md:h-28 md:w-28", rotate: 12 },
    { src: L, className: "absolute left-[5%] bottom-10 h-12 w-12 md:h-16 md:w-16", rotate: -6, delay: "2s" },
  ],
  minimal: [
    { src: L, className: "absolute -right-4 top-12 h-16 w-16 md:h-24 md:w-24", rotate: 8 },
    ...
    <>
      {items.map((d, i) => {
        ...
        return (
          ...
          >
            {light ? (
              /* Dark backgrounds: render on a warm circular bg so colors are visible */
              <div className={cn(
                "relative h-full w-full rounded-full flex items-center justify-center",
                isLogo ? "bg-background/10" : "bg-background/10"
              )}>
                <img
                  src={d.src}
                  alt=""
                  className={cn(
                    ...
                    animClass,
                    isLogo ? "opacity-30" : "opacity-40",
                    "sepia-[0.3] hue-rotate-[340deg]"
                  )}
                  style={{ animationDelay: d.delay || "0s" }}
                />
              </div>
            ) : (
              /* Light backgrounds: render directly with warm tint */
              <img
                src={d.src}
                alt=""
                className={cn(
                  ...
                  animClass,
                  isLogo ? "opacity-[0.12]" : "opacity-[0.35]",
                  "sepia-[0.6] hue-rotate-[340deg] saturate-[0.7]"
                )}
                style={{ animationDelay: d.delay || "0s" }}
              />
            )}
